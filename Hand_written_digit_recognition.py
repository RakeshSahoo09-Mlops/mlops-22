...
## gamma and c parameter for hyper parameters tunning
...
print(f"\nDigit dataset image size: {digits.images.shape}")
...
img_sizes = [(8,8),(3,3),(5,5),(6,6)]

for size in img_sizes:
    if size == (8, 8):
        print(f"\nModeling based on digits dataset of size: {digits.images.shape}")
    else:
        print(f"\nImage size for digits datasets: {digits.images.shape}")
        ...
        print(f"\nUpdated size of image data for digits datasets: {images.shape}")
    ...
    print(f"Model Accuracy for train, dev & test: \n{acc_df}")

    print(f"\nBest hyper-parameter are: {best_h_params}\n")
    print(f"Accuracy of Dev set with Best hyper-parameter is: {best_acc}\n")
